...
export default class webSocketConnector {
  ...
  _attemptConnection() {
    return new Promise((resolve, reject) => {
      ...
      console.log(`${new Date()} | Attempting connection`);
      ...
      this.connection.on('connect', () => {
        console.log(`${new Date()} | Successful connected to: ${config.server}`);
      ...
      this.connection.on('invokeAction', payload => {
          try {
              ...
              console.log(`${new Date()} | ${payload.device} or ${payload.action} not found on the device. Sorry`);
              console.error(e);
          }
      ...
      this.connection.on('disconnect', () => {
        console.log(`${new Date()} | Disconnected from: ${config.server}`)
      })
    });
  }
  ...
  _attemptRegistration() {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`${new Date()} | Remote not found on the server. Attempting registration`);
      ...
      request.post({url:`${config.server}/api/v1/new-remote`, form: {deviceId:this._getDeviceId()}}, (error, responseCode, body) => {
        ...
        fs.writeFile(keyLocation, response.key, (err) => {
          if(err) {
            console.log(`${new Date()} | Writing error`, err);
            return reject('Writing error')
          }
          console.log(`${new Date()} | The file was saved!. Retrying`);

              this._attemptConnection().then(resolve, error => {
                  reject('Connection failed after registration. Aborting');
                  console.log(`${new Date()} | Connection failed after registration. Aborting`, error)
              });
